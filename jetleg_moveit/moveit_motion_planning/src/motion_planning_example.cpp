...
int main(int argc, char* argv[])
{
  ...
  // Set a target joint state
  auto const target_joint_state1 = [] {
    std::map<std::string, double> joint_state;

    joint_state.insert(std::pair<std::string, double>("vertical_rail_to_mount", 0.0));
    joint_state.insert(std::pair<std::string, double>("knee_joint", 0.0));
    joint_state.insert(std::pair<std::string, double>("ankle_joint", 0.0));

    return joint_state;
  }();
  move_group_interface.setJointValueTarget(target_joint_state1);
